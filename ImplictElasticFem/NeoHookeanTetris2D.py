...
while(time < timeFinal):
    ...
    for ie in range(elem_num):
        ...
        area = 0.5
        # 计算力
        H = - area * np.dot(piola,elem_minv[ie])
        dforce = np.zeros((3,2))
        ...
        node_force[elem_idx[ie,0],:] += energy * dforce[0,:] / sumdforce
        node_force[elem_idx[ie,1],:] += energy * dforce[1,:] / sumdforce
        node_force[elem_idx[ie,2],:] += energy * dforce[2,:] / sumdforce
    gravity = np.array([0,-0.2])

    for ip in range(node_num):
        x0 = node_pos[ip,:]
        x1 = np.array([-1,-1],dtype = float)
        x2 = np.array([5,-1],dtype =float)

...
public:
	...
	void ComputeShearForces(Vector3f wu, Vector3f wv, Vector3f dwu, Vector3f dwv, Vector3f p0, Vector3f p1, Vector3f p2, int idx0, int idx1, int idx2)//, MatrixXf dfdx)
	{
		...
		/*
		float dcdt = dcdx0.dot(p0) + dcdx1.dot(p1) + dcdx2.dot(p2);

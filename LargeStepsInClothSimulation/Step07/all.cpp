...
void DrawCloth()
{
	...
	{
		Vector3f p1 = node_pos[element[it * 3 + 0]];
		Vector3f p2 = node_pos[element[it * 3 + 1]];
		Vector3f p3 = node_pos[element[it * 3 + 2]];
		glColor3f(1,1,1);
		glVertex3f(p1(0), p1(1), p1(2));
		glVertex3f(p2(0), p2(1), p2(2));
...
void subStep()
{
	...
	for (int i = 0; i < node_num; i++)
	{
		forces[i * 3 + 0] = rand() % 1000 * 0.001 - 0.4995;
		forces[i * 3 + 1] = -0.05;
		forces[i * 3 + 2] = rand() % 1000 * 0.001 - 0.4995;
	}
